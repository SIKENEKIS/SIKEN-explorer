...
angular.module('insight.contracts')
	...
	.factory('Contracts',
	function(SIKENCoreLib, Opcodes, Networks, Constants) {

		var CONTRACT_CALL = 194;
		var CONTRACT_CREATE = 193;

		return {
			isValidSIKENAddress: function (address) {
                return SIKENCoreLib.Address.isValid(address, Constants.NETWORK)
			},
			getBitAddressFromContractAddress: function (contractAddress) {
				...
				try {
                    ...
                    var network = Networks.getCurrentNetwork(),
                        ...
                        checksum = SIKENCoreLib.crypto.Hash.sha256sha256(new SIKENCoreLib.deps.Buffer(networkId + contractAddress, 'hex')),
                        hexBitAddress = networkId + contractAddress + checksum.toString('hex').slice(0, 8);

                    return SIKENCoreLib.encoding.Base58.encode(new SIKENCoreLib.deps.Buffer(hexBitAddress, 'hex'));
            ...
            getEthAddressFromBitAddress: function (bitAddress) {
				...
				try {
                    ...
                    var network = Networks.getCurrentNetwork(),
                        networkId = network.pubkeyhash.toString(16),
                        hexBitAddress = SIKENCoreLib.encoding.Base58.decode(bitAddress).toString('hex');
			...
			getContractOpcodesString: function (hex) {
				...
				var contractCode = new SIKENCoreLib.deps.Buffer(hex, 'hex'),
					ops = [];
			...
			getContractBytecode: function (hex) {
				...
				try {
					...
					var script = SIKENCoreLib.Script(hex);
			...
			getContractAddress: function (txId, num) {
				var reverseTxId = txId.match(/.{2}/g).reverse().join(""),
					buf = new SIKENCoreLib.deps.Buffer(4);

				buf.writeUInt32LE(num, 0);

				var nHex = buf.toString('hex'),
					addr = reverseTxId + nHex,
					bufferAddress = SIKENCoreLib.crypto.Hash.sha256ripemd160(new SIKENCoreLib.deps.Buffer(addr, 'hex'));
